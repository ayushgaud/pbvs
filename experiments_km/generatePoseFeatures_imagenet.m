function generatePoseFeatures_imagenet(proto, suffix, inputSize, classInd, mirror)

% Generates pose features by running a forward pass of the net, given the
% prototxt file containing the net specification, and the caffemodel file.

% proto: subfolder in {$basedir}/prototxts that contains 'deploy.prototxt'
% for the network to be used
% suffix: subfolder in {$basedir}/cachedir/snapshots/finalSnapshots that
% contains the trained net, i.e., the .caffemodel file
% inputSize: size of image to be input to the net (height/width)
% classInd: vector containing indices of classes for which features need to
% be extracted
% mirror: whether or not to mirror the images and extract features
...
mkdirOptional(fullfile(cachedir,['rcnnPredsVps'],[proto suff '_imagenet']));
% For each class index chosen (about 12, by default)
for ind = classInd
    % Get the class label as a string
    class = pascalIndexClass(ind)
    % Get the rotation data for the Imagenet dataset
    load(fullfile(rotationImagenetDataDir,class));
    ...
    save(fullfile(cachedir,'rcnnPredsVps',[proto suff '_imagenet'],class),'feat');
end
